...
async function main() {
  try {
    ...
    let matchSummary = await getMatchHistory(userID);
    showMatchResult(userID, matchSummary);
  } catch (err) {
    showAlert(
      "Something went wrong. Refresh the page to view match history."
    );
    console.log(err);
  }
}
...
async function getMatchHistory(userID) {
  try {
    const response = await axios.get(
      `/api/${CST.API_VERSION}/match/result/all?userid=${userID}`
    );
